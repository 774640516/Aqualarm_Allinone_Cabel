...
#define BUTTON_DEBOUNCE_TIME      50   //消抖时间      (n-1)*调用周期
#define BUTTON_CONTINUOS_CYCLE  1     //连按触发周期时间  (n-1)*调用周期
#define BUTTON_LONG_CYCLE       1     //长按触发周期时间  (n-1)*调用周期
#define BUTTON_DOUBLE_TIME      200  //双击间隔时间  (n-1)*调用周期  建议在200-600ms
#define BUTTON_LONG_TIME        5000        /* 持续n秒((n-1)*调用周期 ms)，认为长按事件 */
...
typedef struct button
{
    ...
    char Button_State              ;    /* 按键当前状态（按下还是弹起） */
    char Button_Last_State         ;    /* 上一次的按键状态，用于判断双击 */
    char Button_Trigger_Level      ;    /* 按键触发电平 */
    char Button_Last_Level         ;    /* 按键当前电平 */
